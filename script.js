...
const rudeAnswers = [
    // --- Decisive YES (Toxic / Rude) ---
    "ถามโง่ๆ... หุบปากแล้วไปทำซะ!",
    "กูบอกให้ทำ ก็ทำดิ ยืนบื้ออยู่ทำไม!",
    "ดวงมึงกำลังขึ้น รีบๆ ไปทำซะไอ้เวร!",
    "เออ! ลุยเลย! ทำไมต้องให้กูบอกทุกเรื่องวะ",
    "จัดไป! พังไม่พังกูก็เชียร์ให้มึงทำ",
    "ถ้าใจมึงอยากทำขนาดนี้ ก็ไปทำซะ ไป๊!",
    "เอาเลย! กดสูตรติดแล้ว ลุยมันเดี๋ยวนี้แหละ",
    "กูอนุมัติ! ลุยเลยไอ้สัส อย่ามัวแต่ปอดแหก",
    "ทำเลย! ถ้าไม่ทำวันนี้ ชาตินี้มึงก็ไม่เจริญ",
    "เออ! ใช่! ถูกต้อง! มึงไปทำเดี๋ยวนี้เลย!",
    "ลุยโลดดดด! ไม่ต้องคิดเยอะให้ปวดสมอง",
    "กดปุ่มสตาร์ทเดี๋ยวนี้! ชักช้าอยู่ได้",
    "หมอ(แมว)ดูแล้ว ดวงกำลังขึ้น ทำเลย!",
    "ลุยเลยลูกพี่! มัวแต่ถามเดี๋ยวก็โดนด่าหรอก",
    "พุ่งชนเลย! ถ้าไม่ทำตอนนี้ก็ไปนอนซะ",
    "ทำเลย! อย่าลืมเอาปลาทูมาสังเวยกูด้วยล่ะ",
    "สู้เขาสิวะ! อย่ามัวแต่เห่า ลุยเลย!",
    "เชื่อพี่ พี่เรียนมา... คำตอบคือ 'ต้องทำ!'",
    "ลุยให้สุด! แล้วไปหยุดที่ความรวยไอ้เวร",
    "ไฟเขียวสว่างจ้าขนาดนี้ มึงไปทำเถอะขอร้อง!",

    // --- Decisive NO (Toxic / Rude) ---
    "สภาพ... อย่าหาทำเลยดีกว่า พังแน่!",
    "หยุดเถอะ ถือว่าทำบุญทำทานให้ตัวเอง",
    "โง่ดักดาน! ขืนทำไปเตรียมตัวชิบหายได้เลย",
    "มึงนี่มันตำนานความโง่จริงๆ คำตอบคือ 'ห้ามทำ!'",
    "ชิบหายของแท้! หยุดความคิดมึงเดี๋ยวนี้",
    "กูสมเพชมึงว่ะ คำตอบคือ 'ไม่!' เลิกหวังซะ",
    "ปอดแหกแบบมึง อย่าริอาจทำเรื่องนี้เด็ดขาด",
    "คำตอบคือ 'ไม่!' มัดรวมความโง่แล้วโยนทิ้งไปซะ",
    "กูเห็นอนาคตมึงละ... เละเทะ! อย่าทำ!",
    "เตือนแล้วนะ ถ้าดื้อทำ เตรียมตัวบรรลัยได้เลยไอ้ควาย!",
    "อันตรายระดับ 10 ริกเตอร์ หนีไปซะ ห้ามทำ!",
    "พักก่อนมั้ย? หน้าตาเธอดูบ้งมาก คำตอบคือ 'หยุด!'",
    "อย่าหาทำ! แมวเหนื่อยจะด่าแล้วเว้ย",
    "กลับไปล้างหน้า สวดมนต์ แล้วพับแผนนี้ทิ้งซะ",
    "ทำแล้วเตรียมตัวไปรดน้ำมนต์ 9 วัดได้เลย ห้ามทำ!",
    "กราบล่ะ เลิกคิดเรื่องนี้เถอะ มึงทำไม่ได้หรอก",
    "พังกว่านี้ก็คือซากปรักหักพังแล้ว เลิกซะ!",
    "ถ้าทำลงไป ก็เตรียมตัวขึ้นเขียงได้เลยไอ้เวร",
    "ดวงตกสุดๆ ช่วงนี้ ห้ามทำเด็ดขาด กูเตือนละนะ",
    "เอาปากกามาวงเลยว่า 'บ้ง' แน่นอน หยุดเดี๋ยวนี้!",

    // --- Decisive YES (Sassy / Impactful Slang) ---
    "อยากทำก็ทำไปเลยสิ! ใครห้ามมึงได้",
    "ก็บอกให้ทำไง! หูตึงรึไงฮะ",
    "เออ! ลุยไปเลย ไปให้สุด!",
    "ไฟเขียวแล้ว รีบๆ เหยียบมิดไมล์ไปเลย",
    "ทำซะทีเถอะ กูไหว้ล่ะ จะได้จบๆ",
    "อย่ามัวแต่เห่า ลุยเลยสิ!",
    "ถามอยู่นั่นแหละ ไปทำซะที!",
    "ถ้าไม่ทำวันนี้ ก็ไม่ต้องมาคุยกับกูอีก",
    "กูให้ผ่าน! รีบๆ ไปทำซะ",
    "ทำเลย! ถ้าไม่ทำระวังจะเสียใจไปจนตาย",

    // --- Decisive NO (Sassy / Impactful Slang) ---
    "หยุด! สภาพนี้อย่าฝืนเลย",
    "ถ้าทำนี่คือตำนานบทใหม่ (ในทางที่แย่นะ)",
    "เลิกฝันกลางวันแล้วไปนอนซะ",
    "บ้งระดับจักรวาล ห้ามทำเว้ย!",
    "กูเห็นหายนะมาแต่ไกล หยุดเถอะ",
    "ทำไปก็เหนื่อยเปล่า กูเตือนแล้วนะ",
    "รั้นจริงๆ บอกว่าไม่ ก็คือไม่สิ!",
    "กูบอกให้หยุดไง! ดื้อด้านจริงๆ",
    "พักผ่อนบ้างนะ สมองน่ะ อย่าคิดเรื่องแบบนี้เลย",
    "พังแน่ๆ กูเอาหัวเป็นประกัน ห้ามทำ!",
    "อย่าหาทำ! เดี๋ยวก็มานั่งร้องไห้อีก",
    "เลิกคิดเรื่องนี้ซะ! ก่อนที่ชีวิตจะแย่ไปกว่านี้"
    ...
    // --- Ultra Spicy YES ---
    "เอาเลย! พังขึ้นมาอย่ามาร้องไห้ขี้มูกโป่งให้กูเห็นนะ",
    "รีบไปทำซะที! กูรำคาญขี้หน้ามึงเต็มทนละ",
    "ทำๆ ไปเหอะ! ไม่ตายหรอกมั้ง (ถ้าตายกูก็ไม่เกี่ยว)",
    "หน้าอย่างมึง ถ้าใจกล้าทำ ก็เอาเลย กูรอสมน้ำหน้าอยู่",
    "เออ ลุยเลย! ถ้าดวงแข็งพอก็รอดไป",
    "อนุญาตให้ไปทำยอดวิวความชิบหาย ลุยเลยไอ้เวร!",

    // --- Ultra Spicy NO ---
    "โง่แล้วยังขยัน! หยุดความคิดส้นตีนนี้เดี๋ยวนี้",
    "หน้าม้าน! ดันทุรังทำไปก็มีแต่เจ๊งกับเจ๊ง",
    "บอกว่า 'ไม่!' ฟังภาษาคนไม่รู้เรื่องรึไง ไอ้ควาย",
    "ทำไปก็เป็นได้แค่ขยะเปียก หยุดเหอะ",
    "มึงนี่มันสรรหาแต่เรื่องบรรลัยใส่ตัวเนาะ ห้ามทำ!",
    "กราบตีนกูกูก็ไม่ให้มึงทำ! ชิบหายแน่นอน",

    // --- Regional Dialects Toxic (Isan, South, North) ---
    "เฮ็ดมื้อนี้สิเจริญมื้อหน้า ลุยโลดบักปอบ!",
    "สูสิถ่าหยัง เฮ็ดตั่วล่ะ!",
    "ซำมะแจ๋แท้ บักปอบมึง อย่าเฮ็ด!",
    "สมองมีซำหมา อย่ามาหาเฮ็ดเด้อ!",
    "จัดไปต๊ะ! เปรตเอ้อ ลุยเลย!",
    "ชักช้าอยู่ไซร้ ทำเลยโว้ย!",
    "อ้อร้อจริง! มึงหยุดเลยนะ",
    "ชาติเปรตเอ้อ กูบอกว่าไม่ก็ไม่สิ!",
    "สลิดนักเน้อ มึงบ่ะต้องยะเลย!",
    "วอกแต๊วอกว่า จ๊ะไปยะเน้อ!",

    // --- More Isan / South (Toxic) ---
    "ป้าดติโท้ มึงคือเก่งแท้ เฮ็ดโลดบักหล่า!",
    "ฟ้าวเฮ็ดก่อนกูสิสูนด่ามึงเด้อ!",
    "สิถ่าหาพระบิดามึงติ เฮ็ดตั่วล่ะ!",
    "ดีปานนี้ มึงยังสิถามหาหอกหยัง ลุย!",
    "กูบอกให้เฮ็ดกะเฮ็ดตั่วล่ะ บักปอบ!",
    "ผีสิเจาะปากมึง อย่าหาเฮ็ดฮู้บ่!",
    "สิพาตายโหงตั่วหนิ บักซั่ว!",
    "อย่าเฮ็ด กูขี้คร้านสิไปงานศพมึง",
    "บ่เบิ่งสังขารเจ้าของแน่เน๊าะ เซาซะ!",
    "ตาบอดติ สมองบ่มีกะอย่าดันทุรังเฮ็ด!",
    "แหลกจังฮู้ ชาติเปรตเอ้อ ทำเลยสิโว้ย!",
    "ไซร้มึงช้าจังวะ ลุยเลยตะ!",
    "เปรตเอ้อ จะเอาเรินม้าย ถ้าเอาก็ทำเลย!",
    "หมันแหละ! กูบอกให้ทำก็ทำต๊ะไอ้สาส",
    "มึงจะดึงเช็งทำพรือ ทำๆไปต๊ะเห้อ!",
    "ชาดง่าวจังมึงนิ! หยุดเลยนะ",
    "ทำพรือก็ฉิบหายเหม็ด อย่าหาทำดี้!",
    "อย่าแหลงมาก กูรำคาญ ไม่ต้องทำ!",
    "ถ้ามึงทำ กูจะถีบให้จมดินเลย เลิกต๊ะ!",
    "ชาติหมาเอ้อ บอกว่าไม่ทำก็ไม่ทำสิ!"
];

const answers = [
    // --- Decisive YES (Standard) ---
    "แน่นอนที่สุด ทำเลย!",
    "เป็นไปได้สูงมาก ลุย!",
    "ใช่เลยล่ะ ไม่มีอะไรต้องลังเล",
    "ไร้ข้อกังขา ดำเนินการทันที",
    "ผลลัพธ์จะออกมาดีเยี่ยมแน่นอน ลุย!",
    "ลุยเลย รออะไรอยู่!",
    "ทุกอย่างเป็นใจให้คุณ ทำซะ",
    "คำตอบคือ 'ใช่!' 100%",
    "เวลาที่เหมาะสมที่สุดคือตอนนี้ ลุยโลด",
    "ความสำเร็จรออยู่ตรงหน้า ลงมือทำเลย",
    "เทพเจ้าแมวขอฟันธงว่า 'เอาเลย!'",
    "เป็นไอเดียที่ยอดเยี่ยมที่สุด ลุย!",
    "ก้าวต่อไป อย่าหยุด",
    "การตัดสินใจที่ดีที่สุดในชีวิต ทำเลย",
    "พลังบวกมาเต็ม ลุยให้สุด!",
    "ไฟเขียวผ่านฉลุย! ดำเนินการได้",
    "ไม่มีทางพลาดแน่นอน ทำเลย",
    "โอกาสแบบนี้ไม่ได้มีบ่อยๆ คว้าไว้!",
    "ชนะใสๆ ไม่ต้องสืบ ลุย!",
    "เชื่อมั่นในตัวเอง แล้วทำเลย!",
    "ฟ้าเปิดแล้ว ลุยยย!",
    "อย่ามัวแต่คิด ลงมือทำเดี๋ยวนี้",
    "โชคชะตาบอกว่า 'ใช่' แน่นอน",

    // --- Decisive NO (Standard) ---
    "อย่าทำเด็ดขาด!",
    "หยุดความคิดนั้นซะ ตอนนี้เลย",
    "โอกาสพังพินาศสูงมาก ห้ามทำ!",
    "ไม่มีทางเป็นไปได้ ยกเลิกซะ",
    "ถอยออกมาเดี๋ยวนี้!",
    "แมวส่ายหัวรัวๆ คำตอบคือ 'ไม่!'",
    "หายนะกำลังรออยู่ เลิกคิดซะ",
    "มีอุปสรรคใหญ่รออยู่ อย่าเสี่ยงเด็ดขาด",
    "ยังไม่พร้อมหรอก พับโปรเจกต์ไปเลย",
    "เสี่ยงเกินไป ไม่คุ้มค่าเหนื่อย เลิกซะ",
    "ผลลัพธ์จะเลวร้าย เปลี่ยนแผนเถอะ",
    "เปลี่ยนแผนเถอะ เชื่อข้า คำตอบคือไม่",
    "ทางตัน อย่าดันทุรัง",
    "พังแน่นอน ห้ามทำ!",
    "ล้มเลิกซะเถอะ ไม่รอดหรอก",
    "แค่คิดก็ผิดแล้ว หยุดเดี๋ยวนี้",
    "เสียเวลาเปล่าๆ อย่าทำเลย",
    "โอกาสรอดริบหรี่มาก ห้ามทำ",
    "คำตอบคือ 'ไม่!' แบบไร้เยื่อใย",
    "ดับอนาถแน่นอน หยุดเถอะ",
    "บอกเลยว่า พัง พัง พัง! เลิกคิดซะ",

    // --- Slang / Sassy YES (จึ้งๆ ปังๆ) ---
    "ตัวมารดาคอนเฟิร์ม ลุยเลยลูก!",
    "สวรรค์เปิดทาง ลุยโลดดด",
    "จึ้งมากแม่! เอาเลย!",
    "ปังปุริเย่แน่นอน จัดไป!",
    "เริ่ด! ทำเลยไม่ต้องรอ",
    "เอาปากกามาวงเลยว่า ปัง!",
    "มงลงแน่นอน ลุย!",
    "ทำเลย! ดีงามพระรามแปด",
    "โอเคนัมเบอร์วัน! จัดไป",
    "ลุยเลย! อย่าให้เสียชื่อ",
    "เต็มสิบไม่หัก ทำเลย!",
    "บอกเลยว่า เริ่ดมาก ลุย!",
    "เริ่ดเกินต้าน! ลุยเลยจ้า",
    "เอาให้สุด หยุดที่ความปัง!",
    "จักรวาลจัดสรรมาให้แล้ว ทำเลย!",
    "ชั่วโมงนี้ต้องคุณแล้ว ลุย!",
    "ตำนานบทใหม่กำลังจะเกิด ลุย!",
    "ดีเริ่ดประเสริฐศรี! ทำเดี๋ยวนี้",

    // --- Slang / Sassy NO (ช็อตฟีล) ---
    "ปัดตกค่ะ! ขืนทำเตรียมบ้ง",
    "สภาพ! พับเก็บเข้ากรุไปเลย",
    "ช็อตฟีลมาก ห้ามทำเด็ดขาด!",
    "เตือนแล้วนะ หนีไป!",
    "บ้งมากแม่! เลิกคิดซะ",
    "พักก่อน! เรื่องนี้พักก่อน",
    "สู่ขิตแน่นอน ห้ามทำ!",
    "กริบมาก... อย่าหาทำ",
    "ไม่ผ่านอย่างแรง ถอยด่วน",
    "อย่าเสี่ยงเลย เดี๋ยวจะหาว่าไม่เตือน",
    "พังพินาศแน่ หยุดเดี๋ยวนี้",
    "โป๊ะแตกแน่นอน เลิกทำเถอะ",
    "พังพินาศสิคะรออะไร ห้ามทำ!",
    "กะเทยบอกว่าอย่าหาทำ!",
    "บ้งตั้งแต่หน้าปากซอย ถอยด่วน!",
    "ช็อตฟีลขั้นสุด เลิกหวังซะ",
    "พักค่ะลูก พัก! ถอยออกมา",
    "ไม่ผ่านคิวซี พับโปรเจกต์ด่วน",

    // --- Regional Dialects (Isan, South, North) ---
    "คักอีหลี เฮ็ดโลด!",
    "แม่นแล้ว จัดไปเลยสิ่ว",
    "ป้าดดดด ดีแฮง เอาเลย!",
    "ฟ้าวเฮ็ดฟ้าวรวย ลุย!",
    "อย่าหาเฮ็ดเด้อ พังแท้ๆ",
    "เซาๆๆ บ่เป็นตาเฮ็ดดอก",
    "โอ๊ยยย สิบ้าตาย อย่าเฮ็ด!",
    "หรอยจังฮู้! ทำเลย",
    "เอาเลย พันพรือก็ลุย!",
    "จัดไปเลย อย่าช้าแหม็ด",
    "อ้ายหย่า! อย่าทำเด็ดขาด",
    "อย่าหาหาม! พังเหม็ด",
    "ม้ายเอาๆ ถอยดีหวา",
    "เอาเลยเจ้า เปิงขนาด!",
    "แต๊ๆ นา ยะเลย!",
    ...
    // --- More Isan / South (Standard) ---
    "คักขนาด ถืกใจคัก เฮ็ดเลย!",
    "ฟ้าวเฮ็ดด่วนๆ โชคหล่นทับแล่ว",
    "ลุยโลดพี่น้อง บ่งั้นสิเสียใจเด้",
    "ดีกั่วนี้บ่มีอีกแล่ว จัดไป!",
    "โอปะล่ะ? โอติล่ะ เฮ็ดเลย!",
    "ตาซังคัก อย่าเฮ็ดเด็ดขาดเด้อ",
    "พอส่ำนี้ล่ะ ถอยโลด",
    "ปอบหยุมหัวมึงเอ้ย อย่าเฮ็ด!",
    "ซั่วคักซั่วแน อย่าดันทุรังเด้อ",
    "เห็นลางบ่ดีมาแต่ไกล เซาๆ",
    "หรอยแรง! ห้ามพลาดเด็ดขาด",
    "ผ่านฉลุย ลุยเลยพี่บ่าว",
    "แหล่มเลย! ทำตะ แขบๆเข้า",
    "จังหูเลยความรวย ลุย!",
    "หนัดเหนียนเรื่องนี้ เอาเลย!",
    "ไม่ได้เรื่องเลย อย่าทำติงต๊อง",
    "บัดสีจังฮู้ ถอยดีหวา",
    "พังเหม็ดลูกเหอ อย่าทำ",
    "ม้ายรอดแน่ๆ ห้ามเด็ดขาด",
    "ขืนทำไปก็เจ็บเหม็ด เลิกต๊ะ",
    ...
    `${svgCheck} <div>ถูกต้องที่สุด ลุยเลย!</div>`,
    `${svgCheck} <div>ใช่! ไม่มีอะไรต้องลังเล!</div>`,
    `${svgCheck} <div>จัดไปอย่าให้เสีย!</div>`,
    `${svgCross} <div>อย่าเสี่ยงเด็ดขาด!</div>`,
    `${svgCross} <div>หยุดดันทุรัง! ไม่เวิร์ค!</div>`,
    `${svgCross} <div>พังแน่นอน ห้ามทำ!</div>`,
    `${svgLightning} <div>ลุยเลย ความสำเร็จรออยู่!</div>`
];
        ...
        <div class="cat-status-container">
            <div class="mood-bar-wrapper" title="พลังเวทมนตร์ (Aura)">
                <div class="mood-icon" id="moodIcon">
                    <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" class="mystic-svg">
